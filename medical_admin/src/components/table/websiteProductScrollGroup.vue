<template>
  <div>
    <div class="addScroll">
      <el-button @click="goAddScroll" type="primary">添加轮播</el-button>
    </div>
    <el-table
      :data="websiteIndexScroll"
      style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    scrollInfo() {
      ...
      this.$fetch(
        "http://www.bjytzh.cn/jxc/showOfficialList.thtml?key=bg"
      ).then((res) => {
        console.log(res);
    ...
    del(row) {
      ...
      this.$confirm("确定要删除该轮播吗？")
        .then((_) => {
          this.$post(
            "http://www.bjytzh.cn/jxc/deleteOfficialCarousel.thtml",
