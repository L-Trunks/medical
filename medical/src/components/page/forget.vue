    ...
    methods: {
        update(){
            ...
            this.$refs.loginForm.validate((valid) => {
                if(valid) {
                    this.$post('http://localhost:4000/checkForget',qs.stringify(this.loginForm)).then(res => {
                        console.log(res)
                        if(res.message == 'OK') {
                            this.$post('http://localhost:4000/forgetPass',qs.stringify(this.loginForm)).then(res => {
                                if(res.message == 'OK') {
                                    this.$message({
                                        message: "修改成功",
                                        type: 'success'
