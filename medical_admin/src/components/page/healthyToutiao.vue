<template>
  <div>
    <v-pageTitle vtitle="健康头条列表"></v-pageTitle>
    <div class="clear"></div>
    <el-button
      type="danger"
      @click="delGroup"
    ...
    >
    <div class="addNews">
      <el-button @click="goAddNews" type="primary">添加文章</el-button>
    </div>
    <el-table
      ref="multipleTable"
      :data="news"
    ...
    >
      ...
      <el-table-column label="头条标题" width="120">
        <template slot-scope="scope">{{ scope.row.n_title }}</template>
      </el-table-column>
      <el-table-column prop="name" label="缩略图" width="150">
        <template slot-scope="scope">
          <img v-bind:src="scope.row.n_img" style="height: 100px" />
        </template>
      </el-table-column>
      <el-table-column prop="n_abstract" width="250" label="摘要">
      </el-table-column>
      <el-table-column prop="n_see_num" width="100" label="浏览">
      </el-table-column>
      <el-table-column prop="n_from" width="120" label="作者">
      </el-table-column>
      ...
      <el-table-column fixed="right" width="180" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            plain
            size="small"
            @click="goEdit(scope.row.n_id)"
            >编辑</el-button
          >
          <el-button
            @click.native.prevent="deleteNews(scope.row.n_id)"
            type="danger"
            plain
            size="small"
            >移除</el-button
  ...
  methods: {
    delGroup() {
      ...
      this.$confirm("确定要删除选中文章吗？")
        .then((_) => {
          this.$post(
            "http://localhost:4000/deleteToutiaoAll",
    ...
    newsInfo() {
      ...
      //读取商品列表
      this.$post(
        "http://localhost:4000/showToutiaoList",
        qs.stringify(info)
      ).then((res) => {
        console.log(res);
        this.news = res;
        ...
        this.$post(
          "http://localhost:4000/showToutiaoNum",
          qs.stringify(info)
        ).then((res2) => {
    ...
    deleteNews(row) {
      ...
      this.$confirm("确定要删除该商品吗？")
        .then((_) => {
          this.$post(
            "http://localhost:4000/deleteArticle",
