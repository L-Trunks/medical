<template>
  <div>
    <div class="addAward">
      <el-button @click="goaddAward" type="primary">添加奖品</el-button>
    </div>
    <el-table :data="eggAward" style="width: 100%" v-loading="listLoading">
      ...
      <el-table-column prop="co" label="操作" align="right">
        <template slot-scope="scope">
          <el-button
            ...
            >编辑</el-button
          >
          <el-button @click="del(scope.row.id)" type="danger" plain size="small"
            >移除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    awardInfo() {
      ...
      this.$post(
        "http://www.bjytzh.cn/jxc/prizeList.thtml",
    ...
    del(row) {
      ...
      this.$confirm("确定要删除该轮播吗？")
        .then((_) => {
          this.$post(
            "http://www.bjytzh.cn/jxc/deletePrize.thtml",
