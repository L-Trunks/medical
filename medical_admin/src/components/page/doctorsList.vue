  ...
  methods: {
    delGroup() {  
        ...
        this.$confirm('确定要删除选中医生吗？')
          .then(_ => {
            this.$post('http://localhost:4000/deleteDoctorAll',qs.stringify(this.info)).then(res => {
    ...
    newsInfo(){
      ...
      //读取商品列表
      this.$post('http://localhost:4000/showDoctorList',qs.stringify(info)).then(res => {
       console.log(res)
        this.news = res;
        
        this.listLoading = false;
        this.$post('http://localhost:4000/showDoctorNum',qs.stringify(info)).then(res2 => {
    ...
    deleteNews(row){
      ...
      this.$confirm('确定要删除该医生吗？')
        .then(_ => {
          this.$post('http://localhost:4000/deleteDoctor',qs.stringify(this.map)).then(res => {
            if(res.message == "OK"){
              this.newsInfo();
              this.$message({
                    message: "删除成功",
