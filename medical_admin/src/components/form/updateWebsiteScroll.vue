<template>
  <div>
    ...
    <el-form ref="form" label-position="right" label-width="80px" :model="form">
      <el-form-item label="轮播图" prop="c_img">
        <el-upload
      ...
      <el-form-item label="跳转链接" prop="href_url">
        <el-input v-model="form.href_url"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    scrollInfo() {
      this.$post(
        "http://www.bjytzh.cn/jxc/showOfficial.thtml?id=" + this.$route.query.id
      ).then((res) => {
        this.form = res;
      });
    },
    ...
    onSubmit() {
      ...
      this.$post(
        "http://www.bjytzh.cn/jxc/updateOfficialCarousel.thtml",
