    ...
    methods: {
      ...
      delGroup() {  
          ...
          this.$confirm('确定要删除选中信息吗？')
            .then(_ => {
              this.$post('http://localhost:4000/delAllPhone',qs.stringify(this.info)).then(res => {
                if(res.message == "OK"){
                  this.goodsInfo();
                  this.$message({
                      message: "删除成功",
      ...
      goodsInfo(){
        ...
        //读取商品列表
        this.$post('http://localhost:4000/showAllPhone').then(res => {
      ...
      deleteGood(row){
        ...
        this.$confirm('确定要删除该信息吗？')
          .then(_ => {
            this.$post('http://localhost:4000/delPhone',qs.stringify(this.map)).then(res => {
