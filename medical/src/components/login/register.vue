<template>
    <div class="content">
        ...
        <div class="newUser-box">
          <router-link class="newUser" to="/medical/login">已经有账号了？点击登录</router-link>
        </div>
    ...
    methods: {
      ...
      onSubmit(){
        ...
        this.$refs.loginForm.validate((valid) => {
            if(valid) {
              this.$post('http://localhost:4000/check',qs.stringify(this.loginForm)).then(res => {
                //console.log(res.message)
                  if(res.message == 'OK') {
                    //alert(1)
                    this.$post('http://localhost:4000/register',qs.stringify(this.loginForm)).then(res => {
                      //console.log(res.message)
                        if(res.message == 'OK') {
                            this.$message({
                                message: "注册成功，请登录！",
