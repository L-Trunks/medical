  ...
  <div>
    ...
    >
      ...
      <el-form-item label="简介" prop="d_abstract">
        <el-input
          type="textarea"
          v-model="form.d_abstract"
          placeholder="最多输入140字符"
        ></el-input>
      </el-form-item>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        d_id: [{ required: true, message: "请输入医生ID", trigger: "blur" }],
        d_tel: [{ required: true, message: "请输入电话", trigger: "blur" }],
        d_password: [
  ...
  methods: {
    init() {
      this.$fetch("http://localhost:4000/getAddress").then((res) => {
        this.options = res;
      });
    },
    doctorInfo() {
      ...
      this.$post(
        "http://localhost:4000/showDoctorById",
    ...
    uploadImg(file) {
      ...
      this.$post("http://localhost:4000/upload", fd).then((res) => {
    ...
    onSubmit() {
      ...
      this.$refs.form.validate((valid) => {
        ...
        if (valid) {
          this.$post(
            "http://localhost:4000/updateDoctor",
