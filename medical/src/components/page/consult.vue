  ...
  export default {
    ...
    methods: {
           ...
           // console.log(chatState)
            if(chatState=='true'){
               ...
               // console.log(true)
                ...
                this.$post('http://localhost:4000/getDoctorInfo',qs.stringify(this.info)).then(res => {
                    this.doctorImage = res[0].d_face;
                   
                });
                this.$post('http://localhost:4000/getFamilyInfo',qs.stringify(this.info)).then(res => {
                   // console.log(res)
                    this.familyList = res;
        ...
        chat(){
            ...
            this.$post('http://localhost:4000/getChatContent',qs.stringify(detail)).then(res => {
        ...
        next() {
            ...
            this.$refs.form.validate((valid) => {
                if(valid) {
                    this.$post('http://localhost:4000/startChat',qs.stringify(this.form)).then(res => {
                        if(res.message == 'OK') {
                            this.chatState = true;
                            this.setCookie('chatState',true);
                            this.setCookie('record_group_id',this.form.id);

                            this.$post('http://localhost:4000/updatePatientNum',qs.stringify(this.form)).then(res => {
                               // console.log(res)
                            }); 
        ...
        send(){
            ...
            if(this.textarea !== '') {
                this.$post('http://localhost:4000/send',qs.stringify(detail)).then(res => {
                    if(res.message == 'OK') {
                        this.chatState = true;
                        this.textarea = '';
                        this.chat()
