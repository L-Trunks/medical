  ...
  <div>
    ...
    >
      ...
      <el-table-column label="商品名称">
        <template slot-scope="scope" width="170">{{
          scope.row.g_name
        }}</template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <el-button
            type="info"
            size="small"
            @click="update(scope.row.order_num)"
            >编辑</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog title="修改订单" :visible.sync="dialogFormVisible">
      <el-form ref="updateList">
        ...
        <el-form-item label="运单号" :label-width="formLabelWidth">
          <el-input
            v-model="updateList.tracking_num"
            auto-complete="off"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    optionChange() {
      ...
      this.$post(
        "http://www.bjytzh.cn/jxc/getOrdersList.thtml",
    ...
    update(row) {
      ...
      if (row !== undefined) {
        this.$post(
          "http://www.bjytzh.cn/jxc/ordersById.thtml",
          qs.stringify(order_num)
        ).then((res) => {
          //console.log(res)
          this.updateList = res;
          this.selectOrder();
    ...
    updateSubmit() {
      ...
      this.$post(
        "http://www.bjytzh.cn/jxc/updateOrdersSate.thtml",
        qs.stringify(this.updateList)
      ).then((res) => {
        if (res == 1) {
    ...
    deleteGood(row) {
      ...
      this.$confirm("确定要删除该商品吗？")
        .then((_) => {
          this.$post(
            "http://www.bjytzh.cn/jxc/gLogOff.thtml",
