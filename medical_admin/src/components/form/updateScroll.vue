...
<script>
    ...
    export default {
        ...
        methods:{
            init(){
                this.$post('http://localhost:4000/getDoctor').then(res => {
                    this.options = res
                });
            },
             //商品信息初始化
            scrollInfo() {
                ...
                this.$post('http://localhost:4000/getTheAddress' ,qs.stringify(this.form)).then(res => {
                    console.log(res)
                    this.form = res[0];
               });
            },
            onSubmit() {
                ...
                this.$post('http://localhost:4000/updateAddress',qs.stringify(this.form)).then(res => {
                    if(res.message == "OK") {
                        this.$message({
                            message: "修改成功",
                            type: 'success'
