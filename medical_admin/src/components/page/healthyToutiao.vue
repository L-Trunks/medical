    ...
    methods: {
      delGroup() {  
          ...
          this.$confirm('确定要删除选中文章吗？')
            .then(_ => {
              this.$post('http://localhost:4000/deleteToutiaoAll',qs.stringify(this.info)).then(res => {
      ...
      newsInfo(){
        ...
        //读取商品列表
        this.$post('http://localhost:4000/showToutiaoList',qs.stringify(info)).then(res => {
         console.log(res)
          this.news = res;
          
          this.listLoading = false;
          this.$post('http://localhost:4000/showToutiaoNum',qs.stringify(info)).then(res2 => {
      ...
      deleteNews(row){
        ...
        this.$confirm('确定要删除该商品吗？')
          .then(_ => {
            this.$post('http://localhost:4000/deleteArticle',qs.stringify(this.map)).then(res => {
